...
# Run daemon manually
uv run pika-core                        # foreground; Ctrl+C to stop
Pika_PORT=8000 uv run pika-core        # override port
...
This is a **dual-process** local AI agent system. `pika-core` is a persistent daemon; `pika` and `pika-tui` are clients that connect to it over a Unix domain socket.

```
pika-core (daemon)
  └─ listens on 127.0.0.1:7437 (TCP)
       ↑ JSON-RPC 2.0 NDJSON
pika (CLI)   pika-tui (TUI, S2+)
```

**`pika-tui` is the primary frontend.** All user-facing work on task management, observability, and interaction should be designed for and validated in the TUI first. The `pika` CLI exists only for quick scripted testing and debugging — it is not a product surface. When implementing features that touch the user interface, invest in the TUI layout, event rendering, and keyboard interactions. Do not shortcut TUI work by pointing to the CLI as an alternative.

### Protocol layer (`src/Pika_claude/core/bus/`)
...
### Transport layer (`src/Pika_claude/core/transport/`)
...
### Config (`src/Pika_claude/core/config.py`)

Four-tier priority: **built-in defaults → `~/.pika/config.toml` → `.env` → env vars**.
...
Relevant env vars: `Pika_CONFIG`, `Pika_HOST`, `Pika_PORT`, `Pika_LOG_LEVEL`, `Pika_LOG_FILE`, `Pika_LOG_FORMAT`.

### Daemon entry (`src/Pika_claude/core/app.py`)
...
Integration tests in `tests/conftest.py` spawn a real daemon subprocess using a random free port (via `free_port` fixture). The fixture finds a free port, releases it, passes it to the daemon via `Pika_PORT`, then polls `asyncio.open_connection` until the daemon is ready.
