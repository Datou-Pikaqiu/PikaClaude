...
_PID_FILE = Path.home() / ".pika" / "pika-core.pid"


# 尝试连接 daemon，成功则正常返回，失败则抛出 ConnectionRefusedError/OSError
async def _ping_check(config: PikaConfig) -> None:
...
def cmd_core_status(config: PikaConfig) -> None:
    try:
...
def cmd_core_start(config: PikaConfig) -> None:
    try:
        asyncio.run(_ping_check(config))
    ...
    proc = subprocess.Popen(
        [sys.executable, "-m", "pika_claude.core"],
...
def cmd_core_stop(config: PikaConfig) -> None:
    pid = _running_pid()
    if pid is None:
        print("not running")
        return
