...
import pika_claude
from pika_claude.core.bus.commands import (
    AgentRunCommand,
    AgentRunResult,
...
from pika_claude.core.bus.envelope import EventPushEnvelope
from pika_claude.core.config import PikaConfig, get_config
from pika_claude.core.events.bus import EventBus
from pika_claude.core.llm.provider import AnthropicProvider
from pika_claude.core.logging_setup import setup_logging
...
from pika_claude.core.permissions.storage import load_policy_file
from pika_claude.core.runner import AgentRunner
from pika_claude.core.runs import events_file, new_run_id
from pika_claude.core.session import SessionManager, SessionStore
from pika_claude.core.trace.record import TraceRecord
from pika_claude.core.trace.writer import TraceWriter
from pika_claude.core.transport.ipc_broadcaster import IpcEventBroadcaster
from pika_claude.core.transport.socket_server import SocketServer, get_connection_writer
...
class CoreApp:
    def __init__(self) -> None:
        ...
        self._config: PikaConfig | None = None
    ...
    async def _ping_handler(self, params: dict[str, Any]) -> PongResult:
        ...
        return PongResult(
            server_version=pika_claude.__version__,
    ...
    async def _replay_events(
        ...
        if not path.exists():
            for candidate in Path("~/.pika/sessions").expanduser().glob(
    ...
    async def run(self) -> None:
        ...
        policy_file = Path("~/.pika/policy.toml").expanduser()
        ...
        sessions_root = Path("~/.pika/sessions").expanduser()
        ...
        logger.info("pika-core %s listening addr=%s", pika_claude.__version__, addr)
        ...
        # Windows 不支持 add_signal_handler，只在非 Windows 上注册信号
