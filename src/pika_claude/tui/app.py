...
from pika_claude.core.transport.socket_client import IpcError, SocketClient
...
class PikaTuiApp(App[None]):
    """PikaClaude TUI：终端滚屏风格，实时展示 agent 执行过程。"""
    ...
    def compose(self) -> ComposeResult:
        yield Label("[bold]PikaClaude[/bold]  [dim]connecting...[/dim]", id="header")
    ...
    def _update_header(self, state: str) -> None:
        ...
        header.update(
            f"[bold]PikaClaude[/bold]  [dim]{self._host}:{self._port}[/dim]"
    ...
    async def _socket_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                header.update(f"[bold]PikaClaude[/bold]  [red]subscribe error: {e}[/red]")
            finally:
...
# TUI 入口：读取配置并启动 PikaTuiApp
def run(config: PikaConfig, replay_run_id: str | None = None) -> None:
    app = PikaTuiApp(config.host, config.port, replay_run_id=replay_run_id)
    app.run()
